    ...
    <div className="bg-ink text-offwhite">
      <section className="min-h-screen px-4 pt-44 sm:px-6">
        <div className="mx-auto max-w-7xl">
          ...
          <motion.img variants={clipReveal} initial="hidden" animate="visible" className="mt-10 h-[56vh] w-full object-cover" src={images[0]} alt="Mad'ora men's lookbook" />
        </div>
      </section>
      <section className="px-4 py-24 sm:px-6">
        <div className="mx-auto grid max-w-7xl gap-5 md:grid-cols-5">
          {images.slice(1).map((image, index) => (
            <motion.div className={`${index % 3 === 0 ? 'md:col-span-3' : 'md:col-span-2'} overflow-hidden bg-charcoal`} variants={clipReveal} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.25 }} key={image}>
              <img className="h-[560px] w-full object-cover transition duration-700 hover:scale-105" src={image} alt="" loading="lazy" />
            </motion.div>
          ))}
        </div>
      </section>
